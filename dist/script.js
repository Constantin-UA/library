...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.createModal = function ({
  ...
  for (let i = 0; i < this.length; i++) {
    ...
    for (let j = 0; j < btns.count; j++) {
      ...
      let btnWork = btns.settings[j];
      btn.classList.add('btn', ...btnWork[1]);
      btn.textContent = btnWork[0];

      if (btnWork[2]) {
        btn.setAttribute('data-close', 'true');
      }

      if (btnWork[3] && typeof btnWork[3] === 'function') {
        btn.addEventListener('click', btnWork[3]);
      }
...
_core__WEBPACK_IMPORTED_MODULE_0__["default"].prototype.getAttribute = function (...atrName) {
  for (let i = 0; i < this.length; i++) {
    if (!this[i].getAttribute) {
      continue;
    }

    this[i].getAttribute(...atrName);
  }
...
Object(_lib_lib__WEBPACK_IMPORTED_MODULE_0__["default"])('#trigger').click(() => Object(_lib_lib__WEBPACK_IMPORTED_MODULE_0__["default"])('#trigger').createModal({
  text: {
    title: 'Modal title',
    body: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Harum minus doloremque nesciunt enim rem quam corporis? Dolorem pariatur magnam distinctio perferendis.'
  },
  btns: {
    count: 3,
...
Object(_lib_lib__WEBPACK_IMPORTED_MODULE_0__["default"])('tab-panel').tab();
